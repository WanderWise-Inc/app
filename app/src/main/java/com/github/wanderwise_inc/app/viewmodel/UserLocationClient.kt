 ...
 *
 * This class implements the `LocationClient` interface and uses the `FusedLocationProviderClient`
...
class UserLocationClient(
   ...
   *
   * This method returns a Flow that emits Location objects. Each Location object represents the
   * device's location at a particular point in time. The updates are emitted at the interval
   ...
   * @throws LocationClient.LocationException If location permissions are denied or if there is no
   *   mean of tracking position.
   */
