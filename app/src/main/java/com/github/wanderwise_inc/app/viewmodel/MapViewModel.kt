...
class MapViewModel : ViewModel() {
  ...
  public fun getPublicItineraries(): Flow<List<Itinerary>> {
    return flow {
      val snapshot = itineraryCollection.whereEqualTo("visible", true).get().await()
    ...
    return flow {
      val snapshot =
          itineraryCollection
              ...
              .get()
              .addOnFailureListener { e -> Log.d(LOG_DEBUG_TAG, "Get failure: $e") }
              .await()
      val itineraryList = snapshot.map { document -> document.toObject(Itinerary::class.java) }
      emit(itineraryList)
    }
  ...
  public fun setItinerary(itinerary: Itinerary) {
    ...
    docRef
        ...
        .addOnFailureListener { e -> Log.d(LOG_DEBUG_TAG, "Set failure: $e") }
  }
