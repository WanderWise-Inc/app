...
fun HomeNavGraph(navController: NavHostController, innerPadding: PaddingValues) {
  NavHost(
      ...
      startDestination = TopLevelDestination.Overview.route) {
        composable(route = TopLevelDestination.Overview.route) { OverviewScreen() }
        composable(route = TopLevelDestination.Itineraries.route) { ItineraryScreen() }
        composable(route = TopLevelDestination.Map.route) { MapScreen() }
      }
}
