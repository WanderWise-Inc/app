...
class UserViewModel : ViewModel() {
  ...
  public suspend fun setUser(user: User): Boolean {
    ...
    return try {
      usersCollection.document(user.userid).set(u).await()
      true
    } catch (e: Exception) {
  ...
  public fun deleteUser(user: User): Boolean {
    return try {
      usersCollection.document(user.userid).delete()
      true
    } catch (e: Exception) {
  ...
  public fun updateUserName(user: User, newUserName: String): Boolean {
    return try {
      usersCollection.document(user.userid).update("username", newUserName)
      true
    } catch (e: Exception) {
  ...
  public fun updatePhoneNumber(user: User, newPhoneNumber: String): Boolean {
    return try {
      usersCollection.document(user.userid).update("phoneNumber", newPhoneNumber)
      true
    } catch (e: Exception) {
  ...
  public fun updateCountry(user: User, newCountry: String): Boolean {
    return try {
      usersCollection.document(user.userid).update("country", newCountry)
      true
    } catch (e: Exception) {
  ...
  public fun updateDescription(user: User, newDescription: String): Boolean {
    return try {
      usersCollection.document(user.userid).update("description", newDescription)
      true
    } catch (e: Exception) {
      Log.d(TAG, "Error changing country", e)
      false
  ...
  public fun updateEmail(user: User, newEmail: String): Boolean {
    return try {
      usersCollection.document(user.userid).update("email", newEmail)
      true
    } catch (e: Exception) {
  ...
  public fun updateUpVotes(user: User): Boolean {
    return try {
      usersCollection.document(user.userid).update("upVotes", user.upVotes + 1)
      true
    } catch (e: Exception) {
  ...
  public fun fetchImage(bitMapState: MutableState<Bitmap?>, userViewModel: UserViewModel): Bitmap? {
    ...
    profilePictureRef
        ...
        .addOnFailureListener { e -> Log.w("BITMAP", e) }
