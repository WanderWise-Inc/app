...
sealed class Destination(val route: String, val icon: Int, @StringRes val textId: Int) {
  sealed class TopLevelDestination(route: String, icon: Int, textId: Int) :
    ...
    data object Overview :
        TopLevelDestination(
            route = TopLevelRoute.OVERVIEW, icon = R.drawable.menu_icon, textId = R.string.overview_string)

    data object Liked :
        TopLevelDestination(
            route = TopLevelRoute.LIKED, icon = R.drawable.liked_icon, textId = R.string.liked_string)
    ...
    Destination("Creation/$route", icon, textId) {
      data object ChooseLocations :
          CreationStepsDestinations(
              route = CreationStepsRoute.LOCATIONS, icon = R.drawable.location_icon, textId = R.string.locations_string
          )

      data object ChooseDescription :
          CreationStepsDestinations(
              route = CreationStepsRoute.DESCRIPTION, icon = R.drawable.chat_icon, textId = R.string.description_string
          )
