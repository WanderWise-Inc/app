...
class ItineraryRepositoryFirestoreImpl : ItineraryRepository {
    ...
    return flow {
      val snapshot =
          itineraryCollection
              ...
              .get()
              .addOnSuccessListener { Log.d(LOG_DEBUG_TAG, "Get success") }
              .addOnFailureListener { e -> Log.d(LOG_DEBUG_TAG, "Get failure: err=$e") }
              .await()
  ...
  override fun getUserItineraries(userUid: String): Flow<List<Itinerary>> {
    ...
    return flow {
      ...
      val query =
          if (currUserUid != null && currUserUid == userUid) {
  ...
  override fun getItinerariesWithTags(tags: List<Tag>): Flow<List<Itinerary>> {
    return flow {
      val query = itineraryCollection.whereArrayContainsAny(ItineraryLabels.TAGS, tags)
  ...
  override fun setItinerary(itinerary: Itinerary) {
    ...
    docRef
        .set(itinerary.toMap())
        .addOnSuccessListener { Log.d(LOG_DEBUG_TAG, "Set success") }
  ...
  override fun deleteItinerary(itinerary: Itinerary) {
    ...
    docRef
        .delete()
        .addOnSuccessListener { Log.d(LOG_DEBUG_TAG, "Delete success: uid=$itinerary.uid") }
