...
class ItineraryRepositoryTest {
    ...
    private val locationsList = listOf(itineraryObject.SAN_FRANCISCO, itineraryObject.SWITZERLAND, itineraryObject.TOKYO)
    ...
    @Test
    fun `getItinerariesWithTags should return correct list of itineraries with following tags`() = runTest {
        for (i in locationsList.indices) {
            itineraryRepository.setItinerary(locationsList[i])
        }
    ...
    fun `update Itinerary should correctly update the itinerary in the list`() = runTest {
        ...
        assertTrue(getList.containsAll(listOf(itineraryObject.SWITZERLAND, itineraryObject.SAN_FRANCISCO)))
