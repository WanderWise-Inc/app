...
class UserLocationClientTest {
  ...
  @Mock private lateinit var client: FusedLocationProviderClient
  ...
  fun `getLocationUpdates returns correct location`() = runBlocking {
    ...
    `when`(
            context.checkPermission(
                eq(Manifest.permission.ACCESS_COARSE_LOCATION), anyInt(), anyInt()))
  ...
  fun `getLocationUpdates throws correct error when interval is invalid`() {
    assertThrows<IllegalArgumentException> { userLocationClient.getLocationUpdates(0) }

    assertThrows<IllegalArgumentException> { userLocationClient.getLocationUpdates(-1) }
  }
